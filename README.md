...
- If an image's width is not a multiple of 604, you won't be able to select the whole width of the image (the rightmost `width % 604` pixels are cropped off so the banner can be exported at native resolution with no scaling)
